...
En aquesta secció realitzarem un contrast multivariant de dues mitjanes, entre les subpoblacions d'estudiants que fan servir eines de pagament i els que no.
Primer feim el test amb la funció predefinida de R:
```{r, echo = FALSE}
library(Hotelling)
...
on  
$$ \hat{\mathbf{S}} = \frac{n_1 \mathbf{S}_1 + n_2 \mathbf{S}_2}{n_1 + n_2 - 2} $$
i $S_1, S_2$ són les matrius de covariància poblacional per cada població. 
